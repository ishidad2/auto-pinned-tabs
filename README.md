...
- **簡単設定:** 拡張機能アイコンをクリックしてポップアップを開き、対象とするブックマークフォルダをプルダウンで選択するだけで設定完了です。
...
- TypeScriptの型エラーをチェックします（ビルドは行いません）。
...
│       ├── index.html     # ポップアップ画面（フォルダ選択UI）
│       └── main.ts        # ポップアップのロジック
├── components/
│   └── FolderSelector.ts  # 再利用可能なフォルダ選択コンポーネント
├── utils/
...
- **FolderSelectorコンポーネント**: ブックマークフォルダの読み込み、表示、選択、保存のロジックを集約した再利用可能なコンポーネント。デバウンス処理やエラーハンドリングなどの共通機能を提供します。
- **ポップアップUI**: FolderSelectorコンポーネントを使用してシンプルに実装されています。
...
- フォルダを変更したい場合は、再度ポップアップから別のフォルダを選択してください。
...
- **XSS攻撃防止:** `javascript:` スキームのブックマークは自動的にブロックされます。また、DOMへの動的挿入には`textContent`を使用してXSS攻撃を防止します。
...
[GitHub Sponsors でサポートする](https://github.com/sponsors/ishidad2)
...
詳しい開発フローやリリースプロセスについては、[CONTRIBUTING.md](CONTRIBUTING.md)をご覧ください。
